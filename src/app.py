...
if ss.search_result is not None:
    ...
    st.write(f'Days of Sales Outstanding: {"{:.1f}".format(days_sales_outstanding)}%')
    ...
    st.write(f'Days of Payable Outstanding: {"{:.1f}".format(days_payable_outstanding)}%')
    ...
    if days_inventory is not None:
        ...
        st.write(f'Days of Inventory: {"{:.2f}".format(days_inventory)}%')
    ...
    if cash_conversion_cycle is not None:
        ...
        st.write(f'Return on equity: {"{:.2f}".format(cash_conversion_cycle)}%')
        st.write('Return on equity is a measure of how profitable a company is based on how much equity is being invested in')
        st.write('ROE = Net Income / Equity')
        st.write('Higher = Better')
    ...
    st.write('Current asset / current liability')
    st.write('Higher = Better, Danger if less than 1')
    ...
    st.write('Total debt (current and non-current) / equity')
    st.write('Lower = Better, Danger if more than 1')